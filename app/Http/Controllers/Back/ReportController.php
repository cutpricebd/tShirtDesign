...
class ReportController extends Controller
{
    ...
    public function product(Request $request){
        ...
        if($type == 'filter'){
            $categories = Category::where('for', 'product')->orderBy('title')->get();

            return view('back.report.product.index', compact('categories'));
        }else{
            ini_set('memory_limit', '-1');
            ini_set('max_execution_time', 3600);
            ...
            if($request->category){
                $query->whereHas('product_categories', function($q) use($request){
